...
#import common
addon_id='plugin.program.revolutionnews'
...
hubLogo=artp('revolution.jpg'); 
#hubLogo=artp('logo_notice.png'); 
DefaultNoteImage=artp('blank1'); 
DefaultSplitter="|||"; 
DefaultUrl="http://pastebin.com/raw/bKhD9rW0"; 
class MyWindow(xbmcgui.WindowDialog): #xbmcgui.Window): ##xbmcgui.Window
	...
	def __init__(self,noteType='t',noteMessage='',noteImage='',L=140,T=110,W=1000,H=500,Font='font14',TxtColor='0xFF000000'):
		...
		self.BG=xbmcgui.ControlImage(L,T,W,H,self.background,aspectRatio=0,colorDiffuse='0xFFFFFFFFFF'); 
		...
		self.iLogo=xbmcgui.ControlImage((L+(W/2))-(iLogoW/2),T+10,iLogoW,iLogoH+8,hubLogo,aspectRatio=0); 
		## ### ## 
		...
		self.buttonDismiss=xbmcgui.ControlButton(l1,t1,w1,h1,"Dismiss",textColor="0xFFFFFFFF",focusedColor="0xFF000000",alignment=2,focusTexture=focus,noFocusTexture=nofocus); 
		self.buttonRemindMe=xbmcgui.ControlButton(l2,t2,w2,h2,"Remind Later",textColor="0xFFFFFFFF",focusedColor="0xFF000000",alignment=2,focusTexture=focus,noFocusTexture=nofocus); 
		#self.OlaySplash
		...
		for z in [self.BG,self.ImgMessage,self.TxtMessage,self.iLogo,self.buttonRemindMe,self.buttonDismiss]: 
			...
			z.setAnimations([('WindowOpen','effect=zoom delay=0 time=1000 start=0 end=100'),('WindowClose','effect=slide delay=0 time=1000 start=0 end=0,'+str(0-(H+T+10)))]); 
		## ### ## 
