...
addon_id = 'plugin.program.revolutionwiz'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.program.revolutionwiz'
AddonTitle="[COLOR red][R][/COLOR][COLOR white]Evolution Wizard[/COLOR]"
...
PATH = "[R]Evolution Wizard"            
BASEURL = "http://amobox.comlu.com"
H = 'http://'
skin         =  xbmc.getSkinDir()
EXCLUDES2     = ['plugin.program.revolutionwiz','script.module.addon.common','packages']
EXCLUDES     = ['plugin.program.revolutionwiz','script.module.addon.common']
...
def BUILDMENU():
    link = OPEN_URL('http://pastebin.com/raw/6Ta2AtMb').replace('\n','').replace('\r','')
...
def APKDOWNMENU():
    link = OPEN_URL('http://pastebin.com/raw/yUQ28Tbe').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
...
def facebook():
    TextBoxes('[COLOR red][B]Team[/B] ManiaC[/COLOR] [COLOR gold]Wizard[/COLOR]', 'Join the discussions at [COLOR blue]Facebook.com/groups/1510530329266928[/COLOR]')
...
def COMMUNITY_BACKUP():
    ...
    exclude_dirs_full =  ['plugin.program.revolutionwiz']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.revolutionwiz', 'cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
...
def ARCHIVE_FILE(sourcefile, destfile):
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.revolutionwiz' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def WipeXBMC():
    if skin!= "skin.confluence":
        ...
        if choice == 1:
            ...
            title = urllib.quote_plus(vq)
            backup_zip = xbmc.translatePath(os.path.join(mybackuppath,title+'.zip'))
            exclude_dirs_full =  ["plugin.program.revolutionwiz", "plugin.video.adrxbmccustombuildswizard"]
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
            message_header = "Creating full backup of existing build"
            message1 = "Archiving..."
            message2 = ""
